#!/bin/bash
##nswer Script for Project 3 Module 1                   ##
######################################################################
...
# Standard output format: <number>
answer_2() {
	LC_CTYPE=C cut -d  "," -f7 'million_songs_metadata.csv' | grep "Bob Marley" | wc -l 
}
...
# Standard output format: <number>
answer_4() {
	awk 'BEGIN {FS = ","} ; {sum+=$8} END {print sum/1000000}' million_songs_metadata.csv 
}
...
answer_5() {
# LC_CTYPE	join  -1 1 -2 1 million_songs_metadata.csv million_songs_sales_data.csv > million_songs_metadata_and_sales.csv
join -1 1 -2 1 -t ,  million_songs_metadata.csv  million_songs_sales_data.csv > million_songs_metadata_and_sales.csv 
}
...
answer_6() {
	sort -nrk 13 -t ,  million_songs_metadata_and_sales.csv | head -n1 | awk 'BEGIN {FS=","} ;{print $7}'
}
